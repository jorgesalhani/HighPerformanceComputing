...
sem_t  mutex, empty, full; queue_in_use
...
void *producer(void) {
	...
	// Com multiplas produtoras/consumidoras, a variável 'produced'
	// passa a ser crítica. Pode ser que várias produtoras inicializem com produced = 0
	// ao final, todas terão produzidos. Se TOTAL_PRODUCERS > MAX_PRODUCED, estouraria!
...
int main(void) {
	...
	for (int prod_i = 0; prod_i < TOTAL_PRODUCERS; prod_i++) {
	...
	for (int cons_i = 0; cons_i < TOTAL_CONSUMERS; cons_i++) {
